...
#define TEAM_ID "TEAM_04"  // your team ID
...
void mqttReconnect() {
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect(clientId.c_str())) {
      ...
      mqttClient.publish("esp/status", "✅ Connected to MQTT broker test.mosquitto.org");
    } else {
...
void mqttPublish(const String& msg) {
  Serial.println("[MQTT] " + msg);
  if (!mqttClient.connected()) mqttReconnect();
  mqttClient.publish("esp/status", msg.c_str());
}
...
void setup() {
  ...
  if (!xDataMutex) {
    ...
    mqttPublish("❌ Mutex creation failed");
    while (1) delay(1000);
  }
  ...
  if (bmp.begin(0x76) || bmp.begin(0x77)) {
    ...
    mqttPublish("✅ BMP280 initialized");
  } else {
    Serial.println("⚠️ BMP280 not found!");
    mqttPublish("⚠️ BMP280 not found");
  }
  ...
  if (mpu.begin(0x68, &Wire1)) {
    ...
    mqttPublish("✅ MPU6050 initialized");
  } else {
    Serial.println("⚠️ MPU6050 not found!");
    mqttPublish("⚠️ MPU6050 not found");
  }
  ...
  if (!LittleFS.begin(true)) {
    ...
    mqttPublish("❌ LittleFS Mount Failed!");
  } else {
    Serial.println("✅ LittleFS mounted successfully");
    mqttPublish("✅ LittleFS mounted successfully");
  ...
  mqttPublish("✅ Starting calibration...");
  CalibrationTask(NULL);  // Run calibration first (blocking)

  mqttPublish("✅ Calibration complete. Starting other tasks...");
  Serial.println("✅ Calibration complete. Starting other tasks...");

  // Now create other tasks
  xTaskCreatePinnedToCore(SensorTask, "SensorTask", 8192, NULL, 1, NULL, 1);
  xTaskCreatePinnedToCore(XBeeTask, "XBeeTask", 4096, NULL, 2, NULL, 1); // higher no lower the priority
  xTaskCreatePinnedToCore(ServoTask, "ServoTask", 4096, NULL, 1, NULL, 0);
  xTaskCreatePinnedToCore(LoggingTask, "LoggingTask", 4096, NULL, 2, NULL, 1);  // Low priority
}
...
void CalibrationTask(void *pvParameters) {
  ...
  mqttPublish("[Calib] Starting baseline calibration...");
  ...
  if (count > 0) {
    ...
    char msg[100];
    snprintf(msg, sizeof(msg), "[Calib] Baseline altitude = %.2fm", avg);
    mqttPublish(msg);
    Serial.println(msg);
  } else {
    mqttPublish("[Calib] Failed - no valid BMP samples");
...
void SensorTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    float relAlt = NAN;
    if (!isnan(alt_bmp) && !isnan(baselineAltitude))
      relAlt = alt_bmp - baselineAltitude;
...
void XBeeTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    snprintf(out, sizeof(out),
      "%s,%s,%lu,%.2f,%.2f,%.2f,%.6f,%.6f,%.2f,%d,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f",
      TEAM_ID,
      tStr.c_str(),
      pktCount,
...
void ServoTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (!isnan(alt) && alt <= -triggerDrop && !servoBusy) {
      char msg[100];
      snprintf(msg, sizeof(msg), "[Servo] ΔAlt %.2fm -> trigger", alt);
      mqttPublish(msg);
      Serial.println(msg);
...
// ---------- LoggingTask ----------
void LoggingTask(void *pvParameters) {
  const TickType_t logInterval = pdMS_TO_TICKS(1000); // can be faster, but check new data
  SensorData localCopy;
  ...
  for (;;) {
    ...
    if (hasNewData) {
      // Only increment pktCount for new data
      pktCount++;
      ...
      if (LittleFS.begin(false)) {
        ...
        if (file) {
          file.printf("%s,%s,%lu,%.2f,%.2f,%.2f,%.6f,%.6f,%.2f,%d,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f\n",
            TEAM_ID,
            tStr.c_str(),
            pktCount,
            localCopy.altitude_bmp,
          ...
          mqttPublish("[LOG] Data written to server_log.csv");
        } else {
          Serial.println("⚠️ Failed to open log file for writing");
        }
      } else {
        Serial.println("⚠️ LittleFS not mounted!");
      }
    }
