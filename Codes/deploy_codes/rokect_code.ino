...
const char* TEAM_ID = "2024-ASI-063";
...
void apogeeTask(void* pvParameters) {
  ...
  while (true) {
    ...
    if (xQueueReceive(stateQueue, &data, pdMS_TO_TICKS(500)) == pdTRUE) {
      ...
      if (strncmp(currentState, "LAUNCHPAD", 9) == 0 && rel > 2) {
        ...
        if (rel < 10.0f) strncpy(currentState, "IMPACT", sizeof(currentState));
      }
...
void setup() {
  ...
  xTaskCreatePinnedToCore(sdLoggerTask, "sdlog", 4096, NULL, 3, NULL, 1);
  xTaskCreatePinnedToCore(apogeeTask, "apogee", 4096, NULL, 2, NULL, 1);
